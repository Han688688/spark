================================================================================
Apache Spark 核心能力全景分析
================================================================================

版本: Apache Spark 3.5.x
分析时间: 2026-05-18
文档来源: Spark源码 + 官方文档 + API文档
...
Apache Spark是一个统一的大数据分析引擎，提供：
- 统一的编程模型（RDD、DataFrame、Dataset）
- 统一的执行引擎（DAG调度器）
- 统一的资源管理（支持YARN、K8s、Standalone）
...
| 组件 | 位置 | 核心能力 |
|------|------|----------|
| **Spark Core** | core/ | RDD计算引擎，底层执行引擎 |
| **Spark SQL** | sql/ | 结构化数据查询，DataFrame/Dataset |
| **Spark Streaming** | sql/streaming/ | 流式计算（Structured Streaming） |
| **MLlib** | mllib/ | 机器学习库（分类、聚类、回归） |
| **GraphX** | graphx/ | 图计算引擎 |
| **Spark Connect** | sql/connect/ | 远程客户端连接协议 |
| **Resource Managers** | resource-managers/ | YARN、Kubernetes资源管理 |
...
**核心类**: JavaRDD、JavaPairRDD、JavaDoubleRDD
...
- **广播变量(Broadcast Variables)**: 大数据集广播到所有节点
- **累加器(Accumulators)**: 分布式计数器和聚合器
  - LongAccumulator: 长整型累加
  - DoubleAccumulator: 双精度累加
...
**核心类**: SparkSession、DataFrame、Dataset、Column
...
- Pipeline: 特征工程+模型训练流水线
- PipelineModel: 可保存/加载的完整模型
- Transformer: 数据转换器
- Estimator: 模型估计器
- Parameter: 统一参数系统
...
- 客户端-服务器分离架构
- Protobuf协议定义
- DataFrame/Dataset远程操作
- 支持Python/Java客户端
- 无需安装完整Spark

### 7.2 主要API

- SparkSession.connect(): 远程连接
- RemoteDataFrame: 远程DataFrame操作
- Plan serialization: 执行计划序列化
...
- **Standalone**: Spark内置资源管理器
- **YARN**: Hadoop YARN集成
- **Kubernetes**: K8s容器化部署
- **Mesos**: Apache Mesos（已废弃）
- **Local**: 本地单机模式

### 8.2 部署模式

- Client Mode: Driver在客户端
- Cluster Mode: Driver在集群
...
Apache Spark提供：
- ✅ 500+核心API方法
- ✅ 7大核心组件（Core、SQL、Streaming、MLlib、GraphX、Connect、Resource）
- ✅ 20+内置数据源
- ✅ 30+机器学习算法
- ✅ 10+图计算算法
- ✅ 多语言支持（Scala、Java、Python、R、SQL）
- ✅ 多资源管理器（YARN、K8s、Standalone）
- ✅ 完整的大数据生态集成

Spark是大数据领域最强大的统一分析引擎！
