...
**规则**:
- **最少生成**: 3个用例（强制下限，保证基本覆盖）
- **推荐数量**: 5-10个用例（简单场景5-7个，复杂场景10+个）
- **安全上限**: 50个（见generation_limits.yaml，防止失控）
...
**数量**: 0-1个  
**优先级**: P1  
**特点**: 2种error_type同时发生，测试级联失败和隔离恢复

**组合规则**:
- 优先组合共享组件的故障（如Kafka故障+Spark连接Kafka超时）
- 最多组合2种error_type，避免场景爆炸
- 组合用例优先级P1（低于单故障P0，避免混淆）

**示例**: Kafka连接故障 + 数据格式异常同时发生，验证Spark是否先处理数据异常再重试连接。 边缘异常用例（推荐生成）
...
AI从种子用例到新生成用例的变换规则。以下7条（9.1-9.7）是必须执行的泛化策略：
