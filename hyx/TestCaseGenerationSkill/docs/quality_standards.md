...
达标条件:
  ...
  - 验证性分数 ≥ 0.8
  - 总体分数 ≥ 0.8
  - 覆盖率门禁通过（见coverage_dimensions.md 3.5节）
    - 维度覆盖率 ≥ 100%（3必选维度）
    - 组件覆盖率 ≥ 100%（每组件至少1个用例）
    - 路径覆盖率 ≥ 100%（正常流程覆盖所有flow.action）
    - error_type覆盖率 ≥ 50%（适用error_type至少覆盖一半）
    - 总体覆盖率 ≥ 80%
```

**不达标处理**:
- 质量分数不达标 → 在generation_report中标红并附修复建议
- 覆盖率不达标 → 自动触发补充生成循环（见coverage_dimensions.md 3.5节），最多3次
