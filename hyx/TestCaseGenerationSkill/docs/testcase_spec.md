...
### 7b. expected_value合法性校验规则

| 类型 | 合法格式 | 示例 | 非法格式 |
|------|---------|------|---------|
| string | 英文标识符/路径/异常类名/可验证谓词 | `"no_data_loss"`, `"KafkaConnectionException"`, `"file_exists"`, `"retry_mechanism_triggered"` | `"无丢失"`, `"处理成功"`, `"输入1输出1"` |
| number | 正整数或0（count）；0+（data_size）；负值仅range边界 | `100`, `1`, `0` | `-1`(非range), `1.5`(count必须整数) |
| boolean | 仅true/false | `true`, `false` | `"true"`, `"false"`(必须是boolean不是string) |

**校验规则**: expected_value必须匹配上述合法格式。string类型禁含中文字符；count断言必须配整数；boolean断言必须配true/false而非字符串。
